...
set.seed(30)
adonis2(Bac.seq.wu.all ~ species*code+(1/plug.id), data = s.u.samp.df)

# Homogeneity of dispersion test  - no diffs
    set.seed(30)
    permutest(betadisper(Bac.seq.wu.all, s.u.samp.df$code, type = "centroid")) #p=0.004
  TukeyHSD(Bac.seq.wu.all,  ordered = FALSE,conf.level = 0.95)  
...
p<-ggplot(scores.ord.u_RelA_stats, aes(x = NMDS1, y = NMDS2)) + 
  ...
  theme_classic()
ggsave("Initial_NMDS.pdf", plot = p, width = 8, height = 6)
```
