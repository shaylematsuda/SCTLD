...
```{r}
# data.Bac.seq.rel df
Bac.seq.S.rel<-subset_samples (Bac.seq.S.2, experiment=="SCTLD")
...
Bac.seq.S.raw<-subset_samples (Bac.seq.S.2, experiment=="SCTLD")
Bac.seq.S.raw2<-subset_samples(Bac.seq.S.raw, time.point!= "bath")
Bac.seq.S.raw2<-subset_samples(Bac.seq.S.raw2, experiment!= "NTC")
Bac.seq.S.raw2<-subset_samples(Bac.seq.S.raw2, experiment!= "MOCK")
Bac.seq.S.raw2<-subset_samples(Bac.seq.S.raw2, experiment!= "FLU")
Bac.seq.S.raw2<-subset_samples(Bac.seq.S.raw2, species!= "Orbicella faveolata")
Bac.seq.S.raw2<-subset_samples(Bac.seq.S.raw2, species!= "Montastrea cavernosa")
Bac.seq.S.raw2 = prune_taxa(taxa_sums(Bac.seq.S.raw2) > 0, Bac.seq.S.raw2) #this removes any OTU with 0s

Bac.seq.S.raw2.sd <- as(sample_data(Bac.seq.S.raw2), "data.frame") #look at samp data
...
# Extract OTU names
otu_names <- taxa_names(Bac.seq.S.rel2.cnat)
...
core.taxa.standard <- core_members(Bac.seq.S.rel2.cnat, detection = 0.0001, prevalence = .95)
core.taxa.standard

#Try again with genus level
cnat.rel.gen <- aggregate_taxa(Bac.seq.S.rel2.cnat, "Family")
...
# Extract OTU names
otu_names <- taxa_names(Bac.seq.S.rel2.dlab)
...
core.taxa.standard <- core_members(Bac.seq.S.rel2.dlab, detection = 0.0001, prevalence = .95)
core.taxa.standard

#Try again with genus level (lots of unclassified, so at family)
dlab.rel.gen <- aggregate_taxa(Bac.seq.S.rel2.dlab, "Family")
prevalences <- seq(.05, 1, .05)
detections <- round(10^seq(log10(1e-5), log10(.2), length = 10), 3)
par(cex = 0.2)  # Adjust the value (0.8) to make the text smaller
...
Bac.phy.dlab <- Bac.seq.S.rel2.dlab %>% 
  ...
  ##Core Analysis
dlab.core <- core(Bac.seq.S.rel2.dlab, detection = 0.0001, prevalence = .99)
...
#dlab
#dlab <- subset_samples(Bac.seq.S.rel2, species == "Colpophyllia natans")
#dlab.ra <- transform_sample_counts(dlab, function(x) x/ sum(x))

dlab.core <- core(Bac.seq.S.rel2.dlab, detection = 0.0001, prevalence = .97)
dlab.core.taxa <- tax_table(dlab.core)
View(as.data.frame(dlab.core.taxa))
...
Bac.seq.S.rel2.pstr<-subset_samples(Bac.seq.S.rel2.pstr, time.point=="initial")

data.Bac.seq.rel.pstr = prune_taxa(taxa_sums(Bac.seq.S.rel2.pstr) > 0, Bac.seq.S.rel2.pstr) #this removes any OTU with 0s

# Access the OTU table from your phyloseq object
otu_table <- otu_table(Bac.seq.S.rel2.pstr)
...
# Calculate prevalence of taxa
taxa_prevalence <- prevalence(Bac.seq.S.rel2.pstr, detection = 1/100)
tax_table <- tax_table(Bac.seq.S.rel2.pstr)
...
# Count unique taxa
taxa_levels <- as.data.frame(tax_table(Bac.seq.S.rel2.pstr))
unique_species <- n_distinct(taxa_levels$Species)
unique_genera <- n_distinct(taxa_levels$Genus)
unique_families <- n_distinct(taxa_levels$Family)
...
# Extract OTU names
otu_names <- taxa_names(Bac.seq.S.rel2.pstr)
...
core.taxa.standard <- core_members(Bac.seq.S.rel2.pstr, detection = 0.0001, prevalence = .99)
core.taxa.standard

#Try again with genus level (lots of unclassified, so at family)
pstr.rel.gen <- aggregate_taxa(Bac.seq.S.rel2.pstr, "Family")
prevalences <- seq(.05, 1, .05)
detections <- round(10^seq(log10(1e-5), log10(.2), length = 10), 3)
par(cex = 0.2)  # Adjust the value (0.8) to make the text smaller
...
pstr.genus <- aggregate_taxa(Bac.seq.S.rel2.pstr, "Genus")
prevalences <- seq(.05, 1, .05)
detections <- round(10^seq(log10(1e-5), log10(.2), length = 10), 3)
...
#bar plot
#family
Bac.fam.pstr <- Bac.seq.S.rel2.pstr %>% 
  tax_glom(taxrank = "Family", NArm = FALSE) %>% 
  transform_sample_counts(function(x) {x/sum(x)} ) 
  Bac.fam.melt.pstr <- psmelt(Bac.fam.pstr)


  ggplot(Bac.fam.melt.pstr, aes(x = sample_name, y = Abundance, fill = Family)) +
  geom_bar(stat = "identity") +
  #scale_fill_manual(values=pal) +
  #scale_color_manual(values=pal$hex)+
  #scale_fill_manual(values = palette) +  # Use scale_fill_manual() for filling bars
  ...
  #bar plot
#phy
Bac.phy.pstr <- Bac.seq.S.rel2.pstr %>% 
  tax_glom(taxrank = "Phylum", NArm = FALSE) %>% 
  transform_sample_counts(function(x) {x/sum(x)} ) 
  ...
  ##Core Analysis
pstr.core <- core(Bac.seq.S.rel2.pstr, detection = 0.0001, prevalence = .97)
...
#pstr
#pstr <- subset_samples(Bac.seq.S.rel2, species == "Colpophyllia natans")
...
```{r}
Bac.seq.S.rel2.pstr<-subset_samples(Bac.seq.S.rel2, species=="Pseudodiploria strigosa")
Bac.seq.S.rel2.pstr = prune_taxa(taxa_sums(Bac.seq.S.rel2.pstr) > 0, Bac.seq.S.rel2.pstr) #this removes any OTU with 0s
Bac.seq.pstr.f<-subset_samples(Bac.seq.S.rel2.pstr, time.point== "final")
